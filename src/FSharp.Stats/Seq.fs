...
//    // Computes the mean of the means of several subsample
    ...
    let inline covPopulationOfPairs (seq:seq<'T * 'T>) : 'U =
            ...
            |> Array.unzip
            |> fun (x, y) ->
                covPopulation ( x |> Array.toSeq ) (y |> Array.toSeq)


    /// <summary>
    ///   Computes the population covariance of two random variables generated by applying function to the input sequence.
    /// </summary>
    ///    
    ...
    let inline covOfPairs (seq:seq<'T * 'T>) : 'U =
        ...
        |> Array.unzip
        |> fun (x, y) ->
            cov ( x |> Array.toSeq ) (y |> Array.toSeq)

    /// <summary>
    ///   Computes the sample covariance of two random variables generated by applying function to the input sequence.
    /// </summary>
    ///    
    /// <param name="f">A function applied to transform each element of the input sequence into a tuple of paired observations.</param>
    /// <param name="seq">The input sequence.</param>
